...
hid_dim=2048 #1024       # select hidden dimension
...
# LABELS
target=clean #clean|noise
labels_tr=            # use these labels to train (override deafault pdf alignments, has to be in 'Posterior' format, see ali-to-post) 
labels_cv=            # use these labels to cv (override deafault pdf alignments, has to be in 'Posterior' format, see ali-to-post) 
num_tgt=           # force to use number of outputs in the MLP (default is autodetect)
multitask=false    # if true, will concatante feats
multitask_kind=irm # feature type of multi task

# TRAINING SCHEDULER
learn_rate=0.008   # initial learning rate
train_opts=
train_tool=        # optionally change the training tool
train_tool_opts="--minibatch-size=256 --momentum=0.9 --randomizer-seed=777 --randomizer-size=32768"
...
if [ $# != 3 ]; then
   echo "Usage: $0 <data-ali-train> <data-ali-dev> <exp-dir>"
...
echo "config: $hid_layers layers with $hid_dim nodes per layer. Splice is $splice."

for d in $data $data_cv ; do
    for f in {${target},noisy}_$feat_kind.scp ; do
        echo $f
        [ ! -f $d/$f ] && echo "$0: missing $d/$f" && exit 1
    done
    if [ ! -z $aug_train_scp ] ; then
        [ ! -f $d/$aug_train_scp ] && echo "$0: missing $d/$aug_train_scp" && exit 1
    fi
    if $multitask ; then
        [ ! -f $d/$multitask_kind.scp ] && echo "$0: missing $$d/$multitask_kind.scp" && exit 1
    fi
done
echo
echo "# INFO"
...
if ! $skip_cuda_check; then
  cuda-gpu-available || { echo 'CUDA was not compiled in, skipping! Check src/kaldi.mk and src/configure' && exit 1; }
fi

###### PREPARE ALIGNMENTS ######
#echo
#echo "# PREPARING ALIGNMENTS"
if [ ! -z "$labels_tr" ] && [ ! -z "$labels_cv" ] ; then
  echo "Using tr targets '$labels_tr' (by force)"
  echo "Using cv targets '$labels_cv' (by force)"
else
    if [ $aug_fold -gt 0 ]; then
        labels_tr="ark:perl -ne 'for\$i(0..$aug_fold){print}' $dir/ali_train_${target}_$feat_kind.scp | copy-feats scp:- ark:- |"
    else
        labels_tr="ark:copy-feats scp:$dir/ali_train_${target}_$feat_kind.scp ark:- |"
    fi

  labels_cv="ark:copy-feats scp:$dir/ali_cv_${target}_$feat_kind.scp ark:- |"
  if $multitask ; then
    if [ $aug_fold -gt 0 ]; then
        echo "$0: Imcompatiable options --aug-fold and --multitask" && exit 1
    fi
    labels_tr_mt="ark:copy-feats scp:$dir/ali_train_$multitask_kind.scp ark:- |"
    labels_cv_mt="ark:copy-feats scp:$dir/ali_cv_$multitask_kind.scp ark:- |"
    labels_tr="$labels_tr paste-feats-sql ark:- \"$labels_tr_mt\" ark:- |"
    labels_cv="$labels_cv paste-feats-sql ark:- \"$labels_cv_mt\" ark:- |"
  fi
fi
...
for f in noisy_$feat_kind.scp; do
  cat $data/$f | utils/shuffle_list.pl --srand ${seed:-777} > $dir/train_$f
done
for f in ${target}_$feat_kind.scp ; do
  cat $data/$f | utils/shuffle_list.pl --srand ${seed:-777} > $dir/ali_train_$f
  if $multitask ; then
    cat $data/$multitask_kind.scp | utils/shuffle_list.pl --srand ${seed:-777} > $dir/ali_train_$multitask_kind.scp
  fi
done
for f in noisy_$feat_kind.scp; do
  cp $data_cv/$f $dir/cv_$f
done
for f in ${target}_$feat_kind.scp; do
  cp $data_cv/$f $dir/ali_cv_$f
  if $multitask ; then
    cp $data_cv/$multitask_kind.scp $dir/ali_cv_$multitask_kind.scp
  fi
done
if [ ! -z $aug_train_scp ] ; then
    cat $data/$aug_train_scp | utils/shuffle_list.pl --srand ${seed:-777} > $dir/train_$aug_train_scp
    cp $data_cv/$aug_train_scp $dir/cv_$aug_train_scp
fi
# print the list sizes
wc -l $dir/ali_train_${target}_$feat_kind.scp $dir/ali_cv_${target}_$feat_kind.scp
...
#    echo "$0: Not implemented" && exit 1
  ...
  for f in noisy_$feat_kind.scp; do
    mv $dir/train_${f}{,_non_local}; mv $dir/cv_${f}{,_non_local}
    copy-feats scp:$dir/train_${f}_non_local ark,scp:$tmpdir/train_${f%.*}.ark,$dir/train_$f || exit 1
    copy-feats scp:$dir/cv_${f}_non_local ark,scp:$tmpdir/cv_${f%.*}.ark,$dir/cv_$f || exit 1
  done
  for f in ${target}_$feat_kind.scp; do
    mv $dir/ali_train_${f}{,_non_local}; mv $dir/ali_cv_${f}{,_non_local}
    copy-feats scp:$dir/ali_train_${f}_non_local ark,scp:$tmpdir/ali_train_${f%.*}.ark,$dir/ali_train_$f || exit 1
    copy-feats scp:$dir/ali_cv_${f}_non_local ark,scp:$tmpdir/ali_cv_${f%.*}.ark,$dir/ali_cv_$f || exit 1
  done
...
if [ -z $aug_train_scp ] ; then
    if [ $aug_fold -gt 0 ] ; then
        feats_tr="ark:perl -ne 'for\$i(0..$aug_fold){print}' $dir/train_noisy_$feat_kind.scp | copy-feats scp:- ark:- |"
    else
        feats_tr="ark:copy-feats scp:$dir/train_noisy_$feat_kind.scp ark:- |"
    fi
    feats_cv="ark:copy-feats scp:$dir/cv_noisy_$feat_kind.scp ark:- |"
else
feats_tr="ark:cat $dir/train_noisy_$feat_kind.scp $dir/train_$aug_train_scp | copy-feats scp:- ark:- |"
feats_cv="ark:cat $dir/cv_noisy_$feat_kind.scp $dir/cv_$aug_train_scp | copy-feats scp:- ark:- |"
...
#  nnet_init_old=$nnet_init; nnet_init=$dir/nnet_relu.init
#  nnet-copy --binary=false $nnet_init_old - |\
#    sed -e 's/Sigmoid/RectifiedLinear/' |\
#    nnet-copy - $nnet_init || exit 1
fi
if [[ -z "$nnet_init" && -z "$nnet_proto" ]]; then
  ...
  #output-dim
  [ -z $num_tgt ] && num_tgt=$(feat-to-dim "$labels_tr" - )
