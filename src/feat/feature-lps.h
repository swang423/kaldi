...
#ifndef KALDI_FEAT_FEATURE_LPS_H_
#define KALDI_FEAT_FEATURE_LPS_H_
...
/// LpsOptions contains basic options for computing spectrogram
/// features.
struct LpsOptions {
  ...
  void Register(OptionsItf *opts) {
    ...
    opts->Register("energy-floor", &energy_floor,
                   "Floor on energy (absolute, not relative) in Lps computation");
...
class LpsComputer {
 public:
  typedef LpsOptions Options;
  explicit LpsComputer(const LpsOptions &opts);
  LpsComputer(const LpsComputer &other);
 ...
 private:
  ...
  // Disallow assignment.
  LpsComputer &operator=(const LpsComputer &other);
};

typedef OfflineFeatureTpl<LpsComputer> Lps;
...
#endif  // KALDI_FEAT_FEATURE_LPS_H_
