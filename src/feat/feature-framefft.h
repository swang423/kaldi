...
#ifndef KALDI_FEAT_FEATURE_FRAMEFFT_H_
#define KALDI_FEAT_FEATURE_FRAMEFFT_H_
...
/// FrameFftOptions contains basic options for computing spectrogram
/// features.
struct FrameFftOptions {
  ...
  void Register(OptionsItf *opts) {
    ...
    opts->Register("energy-floor", &energy_floor,
                   "Floor on energy (absolute, not relative) in FrameFft computation");
...
class FrameFftComputer {
 public:
  typedef FrameFftOptions Options;
  explicit FrameFftComputer(const FrameFftOptions &opts);
  FrameFftComputer(const FrameFftComputer &other);
  ...
  //int32 Dim() const { return opts_.frame_opts.PaddedWindowSize() * 2; }
 ...
 private:
  FrameFftOptions opts_;
  BaseFloat log_energy_floor_;
  SplitRadixRealFft<BaseFloat> *srfft_;
//  Vector<BaseFloat> phase_frame_, output_frame_;
  int32 padded_window_size_;
  // Disallow assignment.
  FrameFftComputer &operator=(const FrameFftComputer &other);
};

typedef OfflineFeatureTpl<FrameFftComputer> FrameFft;
...
#endif  // KALDI_FEAT_FEATURE_FRAMEFFT_H_
