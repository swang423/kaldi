...
void AddVectorsWithOffset(const Vector<BaseFloat> &signal1, int32 offset,
                           Vector<BaseFloat> *signal2) {
                           //Vector<BaseFloat> *signal2, Vector<BaseFloat> *signal_add) {
  int32 add_length = std::min(signal2->Dim() - offset, signal1.Dim());
//  *signal_add = signal1.Range(0, add_length);
  if (add_length > 0)
...
void AddNoise(Vector<BaseFloat> *noise, BaseFloat snr_db,
                ...
                //BaseFloat signal_power, Vector<BaseFloat> *signal, Vector<BaseFloat> *noise_added) {
  ...
  //AddVectorsWithOffset(*noise, offset, signal, noise_added);
}

void AddNoiseFix(Vector<BaseFloat> *noise, BaseFloat snr_db,
                BaseFloat time, BaseFloat samp_freq,
                BaseFloat signal_power, Vector<BaseFloat> *signal) {
  int32 offset = time * samp_freq;
  int32 add_length = std::min(signal->Dim()-offset,noise->Dim());
  if(add_length > 0){
      float noise_power = VecVec(noise->Range(0,add_length),noise->Range(0,add_length))/add_length;  //only acculumate stats for segment going to be used
      float scale_factor = sqrt(pow(10, -snr_db / 10) * signal_power / noise_power);
      noise->Scale(scale_factor);
      KALDI_VLOG(1) << "Noise signal is being scaled with " << scale_factor
                    << " to generate output with SNR " << snr_db << "db\n";
      signal->Range(offset,add_length).AddVec(1.0,noise->Range(0,add_length));
  }
}
/*
...
int main(int argc, char *argv[]) {
  try {
	...
	std::string output_noise_file;
	if(po.NumArgs()==3)
	  output_noise_file = po.GetArg(3);
    ...
    Matrix<BaseFloat> out_noise_matrix(num_output_channels, num_samp_output);
    for (int32 output_channel = 0; output_channel < num_output_channels; output_channel++) {
//	  KALDI_LOG << "====DEBUG : iter: " << output_channel;
      Vector<BaseFloat> input(num_samp_input);
	  Vector<BaseFloat> noise_output(num_samp_input), noise_mix(num_samp_input);
	  ...
	  Vector<BaseFloat> wav_after_rir(input);
      ...
      if (additive_signal_matrices.size() > 0) {
        ...
        for (int32 i = 0; i < additive_signal_matrices.size(); i++) {
		  if(additive_signal_matrices[i].NumCols()>=input.Dim()){
			...
			noise.SetZero();
			AddVectorsOfUnequalLength(additive_signal_matrices[i].Row(this_noise_channel),&noise);
		  }
          AddNoiseFix(&noise, snr_vector[i], start_time_vector[i],
                    samp_freq_input, early_energy, &input);
                   // samp_freq_input, early_energy, &input, &noise_mix);
//		  noise_output.AddVec(1.0,noise_mix);
        }
      }
	  ...
	  //Get added noise
	  if(po.NumArgs()==3){
		out_noise_matrix.CopyRowFromVec(input,output_channel);
		out_noise_matrix.Row(output_channel).AddVec(-1.0,wav_after_rir);
	  }
...
//		out_noise_matrix.Row(output_channel).Scale(sqrt(power_before_reverb / power_after_reverb));
//We don't normalize the noise matrix as IRM is calculated wrt to the original clean wav
	  }
	...
	if(po.NumArgs()==3){
/*	  if(normalize_output)
		KALDI_WARN << "When normalize_output is true, the noise wav will be incorrect.";
	  Vector<BaseFloat> noise_vec(out_matrix.NumCols());
	  noise_vec.CopyFromVec(out_matrix.Row(0));
	  noise_vec.AddVec(-1.0,input_matrix.Row(input_channel));
	  Matrix<BaseFloat> noise_matrix(1,noise_vec.Dim());
	  noise_matrix.CopyRowFromVec(noise_vec,0);*/
	  WaveData out_noise(samp_freq_input,out_noise_matrix);
	  Output kon(output_noise_file,false);
	  out_noise.Write(kon.Stream());
	}
    return 0;
